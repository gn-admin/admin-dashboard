...
- **Service worker**: al tocar `src/js/dashboard.js`, `api.js` u otros, subir `CACHE_NAME` en `src/sw.js`. Estado actual: `gn-encuestas-v21`.
...
**Desplegado y funcionando** en `https://gn-admin.github.io/admin-dashboard/`. Frontend con rutas relativas; SW `v21`. Mejoras recientes (frontend, pendiente redeploy de backend y hojas): aprobación de encuestas → candidaturas → **Procesos** (asignación de animal/familia), **Acogidas activas** (casos `entrega→en_casa→finalizada`), ficha animal con especie extensible, **grupos/camadas** (alta bulk, grupo obligatorio, filtro por especie) y **contratos de adopción** con firma en canvas + PDF. Persistencia intermedia en localStorage (`gn_candidaturas`, `gn_contratos`, `gn_acogidas`). El fix de aislamiento de estados/notas y `ALLOWED_ORIGINS` con `https://gn-admin.github.io` siguen pendientes de desplegar en el backend. A partir de aquí se trabajan **mejoras de front y back**, sin tocar la lógica de negocio.